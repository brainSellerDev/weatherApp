/*
-создать объект конфиг и все туда вынести(url, key, IP-KEY)
-написать функцию инициализации - для работы с дом деревом, documen.querySelector().....
-все переменные обявить вначале файла (weatherForecastIcon, weatherCondition....)
-функция рендер - должна отрисовывать 
-функция update - обновлять данные 
-стреловчные функци использовать, если в теле функции мало строчек кода 
-в параметры функции не передавать то что является константой (getIpAddress)
-очищать слушатели событый после их срабатываний 
-функция для запросов - должна делать только запрос 
-try-catch - должны пробрасывать ошибку наверх throw
-loader - должен обрабатываться когда вызывается функция для запроса, а не в самой функции сапроса 
-переписать функцию лоадера (принимает статус лоадера и в зависимости от статуса обрабатывается)
-не объединять по хорошему в одну функцию, несколько функций 
- в файле README -описать хорошо весь проект и настройку проекта 
*/
...
//переписать в function declaration
